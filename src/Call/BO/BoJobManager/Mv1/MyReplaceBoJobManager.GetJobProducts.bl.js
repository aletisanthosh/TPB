 ...
 * @function replaceGetJobProducts
 * @this BoJobManager
 * @kind businessobject
 * @async
 * @namespace CUSTOM
 ...
 * @returns promise
 */
function replaceGetJobProducts(posId){
    var me = this;
    ///////////////////////////////////////////////////////////////////////////////////////////////
    //                                                                                           //
...
if (loCurrentPOS.length > 0) {
  ...
  if (liCurrentPOS.getProductsInitialized() !== "1") {
    ...
    liCurrentPOS.setProductsInitialized("1", true);
    ...
    for (idxExistingJobMainSurveys = 0; idxExistingJobMainSurveys < existingJobMainSurveys.length; idxExistingJobMainSurveys++) {
      ...
      currentDicExistingJobsOfProduct[currentSurvey.getJobDefinitionMetaPKey()] = { value: currentSurvey.clbMainPKey == me.getClbMainPKey() ? currentSurvey.getValue() : (Utils.isEmptyString(currentSurvey.getLastValue()) ? currentSurvey.getValue() : currentSurvey.getLastValue()), clbMainPKey: me.getClbMainPKey() };
    }
    ...
    promise = Facade.getListAsync("LoJobProducts", jsonQuery).then(
      function (listJobProducts) {
        for (var idxProduct = 0; idxProduct < listJobProducts.length; idxProduct++) {
          ...
          currentJobProduct.hasChanged = hasChanged;
        ...
        if (listJobProducts.length > 0) {
          ...
          promise = me.myGetAllSurveys(posId);
        }
        liCurrentPOS.setSurveyCount(loProducts.getCount(), true);
      });
  }
  else {
