 ...
 * @function replaceOnQuestionChanged
 * @this BoJobManager
 * @kind businessobject
 * @async
 * @namespace CUSTOM
 ...
 * @returns promise
 */
function replaceOnQuestionChanged(handlerParams, jDTPKey){
    var me = this;
    ///////////////////////////////////////////////////////////////////////////////////////////////
    //                                                                                           //
  ...
  for (var i = 0; i < handlerParams.modified.length; i++){
    ...
    if (value !== oldValue){
      if (columnName == "value"){
        ...
        if(listItem.getDataType() === "Decimal" && Utils.isDefined(listItem.getDecimalMinValue()) && Utils.isDefined(listItem.getDecimalMaxValue()) && listItem.getDecimalMaxValue() != 0) {
          var chosenValue = listItem.getValue();
          if(chosenValue === " " || chosenValue === null){
            chosenValue = listItem.getDecimalMinValue();
          }
          if(!(chosenValue <= listItem.getDecimalMaxValue() && chosenValue >= listItem.getDecimalMinValue())){
            if (oldValue > listItem.getDecimalMaxValue() || oldValue < listItem.getDecimalMinValue()) {
              oldValue = listItem.getDecimalMinValue();
            }
            loQuestions.suspendListRefresh();
            listItem.setValue(oldValue);
            loQuestions.resumeListRefresh(true);
            
            if (done == "0" && listItem.getDone() == "1") {
              listItem.setDone("0");
            }
            ...
            var newError = {
              "level": "error",
              "objectClass" : "BoCall",
              "messageID" : "CasClbPriceMustBeInBetweenMinAndMaxPolicy",
              "messageParams" : {
                "minValue" : listItem.getDecimalMinValue(),
                "maxValue" : listItem.getDecimalMaxValue()
              }
            };
            messageCollector.add(newError);
            var buttonValues = {};
          ...
          if(listItem.getValue() === " " || listItem.getValue() === null){
            listItem.setValue(listItem.getDecimalMinValue());
          }
        }
