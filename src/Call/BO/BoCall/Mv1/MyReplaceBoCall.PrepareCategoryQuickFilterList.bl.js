 ...
 * @returns promise
 */
function replacePrepareCategoryQuickFilterList(load, isPromotionFilterSet, isDiscrepanciesFilterSet){
    var me = this;
    ///////////////////////////////////////////////////////////////////////////////////////////////
    //                                                                                           //
...
var addItemsToLoProductQuickFilter = function (items){
  ...
  for (var i=0; i<items.length; i++){
    categoryExists = loProductQuickFilter.getItemsByParam({"text": items[i].getPrdGroupText()});
    if (categoryExists.length === 0){
      item = {
        "pKey": PKey.next(),
        "text": items[i].getPrdGroupText(),
        "isSelected": false,
        "specialOption": ""
      };
...
if((ifSelectedInList || ifSelectedInCache) && load === "0"){
  ...
  filterArray.push({"prdGroupText": selectedFilter, "op": "EQ"});

  loCurrentSurveyProducts.resetFilter("prdGroupText");
