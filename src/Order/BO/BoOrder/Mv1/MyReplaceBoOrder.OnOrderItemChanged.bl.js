 ...
 * @function replaceOnOrderItemChanged
 * @this BoOrder
 * @kind businessobject
 * @async
 * @namespace CUSTOM
 ...
 * @returns promise
 */
function replaceOnOrderItemChanged(bufferedEvents){
    var me = this;
    ///////////////////////////////////////////////////////////////////////////////////////////////
    //                                                                                           //
...
var quantityRewardProcessTriggers = [];
  ...
  for (var i = 0; i < currentEvent.modified.length; i++) {
    ...
    switch (currentEvent.modified[i]) {
      ...
      case "quantity": {
        ...
        if (
          Utils.isDefined(parentType) &&
          (parentType === "Order" || parentType === "Item")
        ) {
          isPEFreeItem = true;
        }
        ...
        orderUnitItem = me
          .getLoItems()
          .getFirstOrderUnitItemForMainItem(mainItem);
        modReasonTriggerObj = {
          mainItem: mainItem,
        ...
        if (mainItem.getPromoted() === "1") {
          quantityRewardChangedTriggerObj = {
            mainItem: mainItem,
            orderUnitItem: orderUnitItem,
            oldValue: currentEvent.oldValues.quantity,
            newValue: currentEvent.newValues.quantity,
          };
          quantityRewardProcessTriggers.push(quantityRewardChangedTriggerObj);
        }
      ...
      case "specialPriceReceipt": {
        ...
        var basePriceVal = mainItem.getBasePrice();

        if (mainItem.getSpecialPriceReceipt() !== 0) {
          if (
            (basePriceVal === 0 &&
            (mainItem.getSpecialPriceReceipt() < 0 ||
              mainItem.getSpecialPriceReceipt() > 99.99) &&
            mainItem.type === 'Cash Sale') ||
            (mainItem.type === 'Return' &&
            (mainItem.getSpecialPriceReceipt() < 0 ||
              mainItem.getSpecialPriceReceipt() > 99.99))
          ) {
            return MessageBox.displayMessage(
              Localization.resolve("MessageBox_Title_Warning"),
              Localization.resolve("SPPriceValidationNoPrice"),
              buttonValues
            ).then(function () {
              return when(mainItem.setSpecialPriceReceipt(0));
            });
          } else if (
            basePriceVal !== 0 &&
            (mainItem.getSpecialPriceReceipt() < basePriceVal * 0.9 ||
              mainItem.getSpecialPriceReceipt() > basePriceVal * 1.25) &&
            mainItem.type === 'Cash Sale'
          ) {
            return MessageBox.displayMessage(
              Localization.resolve("MessageBox_Title_Warning"),
              Localization.resolve("SPPriceValidation"),
              buttonValues
            ).then(function () {
              return when(mainItem.setSpecialPriceReceipt(0));
            });
...
if (isUpdateRequired) {
  ...
  var qtyChangedHandler = function (x) {
    ...
    var sdoItemMeta = me
      .getBoOrderMeta()
      .getLoOrderItemMetas()
      .getItemByPKey(mainItem.getSdoItemMetaPKey());
    ...
    if (!me.loProductForAdd) {
      promise = promise.then(function () {
        return me.loadProductForAdd();
      });
    }

    promise = promise.then(function () {
      // 1072 - alert and no update to quantity once current type is not orderable - BEGIN
      var mainProduct = me.loProductForAdd
        .getAllItems()
        .find((item) => item.prdMainPKey == x.mainItem.prdMainPKey);
      var mTypeToOrderable = {
        "Cash Sale": mainProduct.cashSaleOrderable == "1",
        Prebook: mainProduct.prebookOrderable == "1",
        Return: mainProduct.returnable == "1",
      };
      if (!mTypeToOrderable[mainItem.type] && oldValue == 0 && newValue > 0) {
        me.getLoItems().suspendListRefresh();
        mainItem.setQuantity(oldValue);
        mainItem.setEdited("0");
        me.getLoItems().resumeListRefresh(true);

        return MessageBox.displayMessage(
          Localization.resolve("MessageBox_Title_Warning"),
          Localization.resolve(
            "QtyEditNotAllowedSinceTypeNotOrderable"
          ).replace("[insert order item type]", mainItem.type + " orderable"),
          buttonValues
        );
      }
      // 1072 - alert and no update to quantity once current type is not orderable - END

      if (
        (me.getBoOrderMeta().getConsiderQuantitySuggestion() === "PQty" ||
          isFreeItem ||
          mainItem.getSpecialPromoted() == "1") &&
        mainItem.getMergeEngine_invalidated() === "0" &&
        mainItem.getEdited() !== "0"
      ) {
        ...
        if (
          ...
          modifyPresetQuantityOption = me
            .getLuDeliveryRecipient()
            .getSdoModifyPresetQuantity();
        } else if (Utils.isDefined(sdoItemMeta)) {
          modifyPresetQuantityOption = sdoItemMeta.getModifyPresetQuantity();
        }
        var setPresetQuantity = function (
          item,
          modifyPresetQtyOption,
          presetValue
        ) {
        ...
        if (
          ...
          return MessageBox.displayMessage(
            ...
            .then(function () {
              me.getLoItems().createDisplayInformationForItem(
                orderUnitItem,
                me.getBoOrderMeta()
              );
            });
        } else {
          return when(mainItem.setEditedQty(mainItem.getQuantity())).then(
            function () {
              me.getLoItems().createDisplayInformationForItem(
                orderUnitItem,
                me.getBoOrderMeta()
  ...
  var pricingUpdateHandler = function (x) {
    ...
    if (
      ...
      pricingPromise = CP.PricingHandler.getInstance()
        .updateProduct(mainItem.getData(), x.modifiedAttribute)
        .then(function () {
          if (isNewItem) {
  ...
  var qtyRewardHandler = function (x) {
    var mainItem = x.mainItem;
    var oldValue = x.oldValue;
    var newValue = x.newValue;
    //check quantity modification allowed
    ...
    if (
      me
        .getLoSelectablePromotion()
        .getItemByPKey(mainItem.getPromotionPKey())
        .getSatisfiedRewardGroupCount() === 0 && 
      mainItem.getLogicUnitsforDisplay() === "Carton"
    ) {
      var isBelowMinimum = false;
      var isExceededMaximum = false;
      me.getHurdleEvaluationHelper()
        .getLoHurdles()
        .getItems()
        .forEach(function (item) {
          if (item.getIsFulfilled() === "0" && item.getOperator() === "ge" && 
              item.getCalculatedValue() < (item.getCalculatedValue() - newValue + oldValue) && 
              (item.getCalculatedValue() - newValue + oldValue) >= item.getTargetValue() && 
              item.getCalculatedValue() < item.getTargetValue()
          ) {
            isBelowMinimum = true;
          } else if (
            item.getIsFulfilled() === "0" &&
            item.getOperator() === "le" && 
            item.getCalculatedValue() > (item.getCalculatedValue() - newValue + oldValue) && 
            (item.getCalculatedValue() - newValue + oldValue) <= item.getTargetValue() && 
            item.getCalculatedValue() > item.getTargetValue()
          ) {
            isExceededMaximum = true;
          }else if (item.getIsFulfilled() === "0" && item.getOperator() === "gt" && 
              item.getCalculatedValue() < (item.getCalculatedValue() - newValue + oldValue) && 
              (item.getCalculatedValue() - newValue + oldValue) > item.getTargetValue() && 
              item.getCalculatedValue() <= item.getTargetValue()
          ) {
      ...
      if (isExceededMaximum) {
        return MessageBox.displayMessage(
          Localization.resolve("MessageBox_Title_Warning"),
          Localization.resolve("QtyRewardMaxValidation"),
          buttonValues
        );
      } else if (isBelowMinimum) {
        return MessageBox.displayMessage(
          Localization.resolve("MessageBox_Title_Warning"),
          Localization.resolve("QtyRewardMinValidation"),
          buttonValues
        );
      }
    }
  ...
  promise = when
    ...
    .then(function () {
      ...
      if (
        ...
        quantityProcessTriggers.length > 0 &&
        !Utils.isEmptyString(
          quantityProcessTriggers[0].mainItem.getPromotionPKey()
        ) &&
        me.getIsNotReadyForHurdleEvaluation() !== "1"
      ) {
        return me
          .getHurdleEvaluationHelper()
          .evaluateHurdles(
            me,
            me.getBoItemTabManager().getBoCallCache(),
            quantityProcessTriggers[0].mainItem.getPromotionPKey()
          )
          .then(function () {
            var promotionPkey =
            ...
            return me
              .getHurdleEvaluationHelper()
              .determineSatisfiedRewardGroups(
                promotionPkey,
                me.getLoSelectablePromotion()
              )
              .then(function () {
                me.updateRewardGroupSatisfactionCounter(promotionPkey);
              })
              .then(function () {
                var qtyRewardChangedDeferreds =
                  quantityRewardProcessTriggers.map(qtyRewardHandler);
                return when.all(qtyRewardChangedDeferreds);
              });
          });
      }
    })
    ...
    .then(function () {
      var pricingUpdateDeferreds =
        calculationProcessTriggers.map(pricingUpdateHandler);
