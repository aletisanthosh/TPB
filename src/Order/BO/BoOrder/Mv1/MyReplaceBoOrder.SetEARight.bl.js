 ...
 * @returns promise
 */
function replaceSetEARight(promotionPKey){
    var me = this;
    ///////////////////////////////////////////////////////////////////////////////////////////////
    //                                                                                           //
...
var acl = me.getACL();
var uom = me.getBoItemTabManager().getLoUnitOfMeasure();
...
var cashItemsSize = 0;
var returnItemsSize = 0;
var prebookItemsSize = 0;
orderItems.forEach(function(item) {
  if((item.getType() === "Cash Sale" || item.getType() === "Free Item") && item.getQuantity() > 0){
    cashItemsSize += 1;
  }else if(item.getType() === "Return" && item.getQuantity() > 0){
    returnItemsSize += 1;
  }else if(item.getType() === "Prebook" && item.getQuantity() > 0){
    prebookItemsSize += 1;
  }
});
if((cashItemsSize > 0 || returnItemsSize > 0) && prebookItemsSize === 0){
  acl.removeRight(AclObjectType.PROPERTY, "deliveryDate", AclPermission.EDIT);
}else if(cashItemsSize === 0 && returnItemsSize === 0 && prebookItemsSize === 0){
  acl.removeRight(AclObjectType.PROPERTY, "deliveryDate", AclPermission.EDIT);
}else{
  acl.addRight(AclObjectType.PROPERTY, "deliveryDate", AclPermission.EDIT);
}
...
if ((phase === BLConstants.Order.PHASE_CLOSED) || (phase === BLConstants.Order.PHASE_CANCELED) || (phase === BLConstants.Order.PHASE_READY) || (phase === BLConstants.Order.PHASE_FEEDBACK) || (phase === BLConstants.Order.PHASE_VOIDED) ||
  ...
  uomAcl.removeRight(AclObjectType.PROPERTY, "wsName", AclPermission.EDIT);
  orderItems.forEach(function(item) {
    item.getACL().removeRight(AclObjectType.PROPERTY, "specialPriceReceipt", AclPermission.EDIT);
    item.getACL().removeRight(AclObjectType.PROPERTY, "wsName", AclPermission.EDIT);
    item.getACL().removeRight(AclObjectType.PROPERTY, "resale", AclPermission.EDIT);
  });
}
else {
...
// switch UI based on order item template "Prebook"
function OrderItem__switchUI(orderItem) {
  function hideFieldUI(field) {
      orderItem.getACL().removeRight(AclObjectType.PROPERTY, field, AclPermission.VISIBLE);
  }
  function showFieldUI(field) {
      orderItem.getACL().addRight(AclObjectType.PROPERTY, field, AclPermission.VISIBLE);
  }
  function showAndReadOnlyFieldUI(field) {
      orderItem.getACL().removeRight(AclObjectType.PROPERTY, field, AclPermission.EDIT);
      orderItem.getACL().addRight(AclObjectType.PROPERTY, field, AclPermission.VISIBLE);
      
  }

  var fnShowUIWhenPrebook = orderItem.type === 'Prebook' ? showFieldUI : hideFieldUI;
  ["wholesalerProduct", "tpbSku", "wsNameDisplay", "wholesalerId", "wholesalerName", "wholesalerProductSKU"].forEach(fnShowUIWhenPrebook);
  
  var fnHideUIWhenPrebook = (orderItem.type !== 'Prebook' && orderItem.type !== 'Free Item') ? showFieldUI : hideFieldUI;
  ["price", "basePrice", "specialPrice", "basePriceReceipt", "valueReceipt", "specialPriceReceipt"].forEach(fnHideUIWhenPrebook);
  
  hideFieldUI('resale');

  // mass prebook to disallow the type field changes since its prebook items are having difference promotion waves.
  if (orderItem.tpbPromoted == '1') {
    showAndReadOnlyFieldUI('type');
  } 
}
orderItems.forEach(OrderItem__switchUI);
uom.forEach(OrderItem__switchUI);
  ...
  promise = Facade.getObjectAsync("BoCall", me.getQueryBy("pKey", me.getCallInContext_clbMainPKey()))
    ...
    if (Utils.isDefined(boCall) && me.getBoOrderMeta().getSdoSubType() === "Delivery") {
      ...
      if(bCallIsReadOnly) {
        ...
        me.getAcl().removeRight(AclObjectType.PROPERTY, "deliveryDate", AclPermission.EDIT);
        me.getLuBrokerCustomer().getACL().removeRight(AclObjectType.OBJECT, "luBrokerCustomer", AclPermission.EDIT);
        me.getLuDeliveryRecipient().getACL().removeRight(AclObjectType.OBJECT, "luDeliveryRecipient", AclPermission.EDIT);
        uomAcl.removeRight(AclObjectType.PROPERTY, "modReason", AclPermission.EDIT);
        uomAcl.removeRight(AclObjectType.PROPERTY, "quantity", AclPermission.EDIT);
