 ...
 * @function replaceSetInventoryBalanceOfItem
 * @this BoOrder
 * @kind businessobject
 * @async
 * @namespace CUSTOM
 ...
 * @returns promise
 */
function replaceSetInventoryBalanceOfItem(itemPKey){
    var me = this;
    ///////////////////////////////////////////////////////////////////////////////////////////////
    //                                                                                           //
      ...
      function (loInventoryFinding){
        ...
        for (var i = 0; i < ivcMetasByItemMeta.length; i++) {
          ...
          if (liInventory.length > 0){
            ...
            for (var idx = 0; idx < liInventory.length; idx++) {
              var inv = liInventory[idx];
              
              // Find the inventory with the same state as the customer store
              if (Framework.getProcessContext().mainBO.luOrderer.shippingState === inv.tourState) {
                ivcInformation.ivcMainPKey = inv.getIvcMainPKey();
                ivcInformation.balance = inv.getBalance();
              }
              
              // The default inventory generated by offline app is not related to any state
              if (Utils.isEmptyString(inv.tourState)) {
                defaultInv = inv;
              }
            }
            
            // If there is not matched state inventory for the customer, use the default one
            if (Utils.isEmptyString(ivcInformation.ivcMainPKey) && defaultInv) {
              ivcInformation.ivcMainPKey = defaultInv.getIvcMainPKey();
              ivcInformation.balance = defaultInv.getBalance();
            }
          }
