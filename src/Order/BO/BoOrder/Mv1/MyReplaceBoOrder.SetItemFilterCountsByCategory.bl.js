 ...
 * @function replaceSetItemFilterCountsByCategory
 * @this BoOrder
 * @kind businessobject
 * @namespace CUSTOM
 */
...
// 230313 guanshu #824
var countDistributed = 0;
var countTpbPromoted=0;
var lastRefPKey;
var i;
...
for (i = 0; i < orderItems.length; i++) {
  ...
  if (orderItem.getGroupId() == currentFilterId && orderItem.getRefPKey() != lastRefPKey) {
    ...
    // 230311 guanshu #562
    if (orderItem.getShowInSuggested() == "1" && orderItem.getQuantity() === 0) {
      if(!Utils.isDefined(orderItems.find(function(item){ return item.quantity !== 0 && item.refPKey === orderItem.refPKey; } ))){
        countSuggested++;
        alreadyCountedItem[orderItem.getRefPKey()] = "";
      }
    }
    // 230313 guanshu #824
    if (orderItem.getDistributed() == "1") {
      countDistributed++;
    }
    if (orderItem.getTpbPromoted() == "1") {
      countTpbPromoted++;
      alreadyCountedItem[orderItem.getRefPKey()] = "";
    }
  }
...
for (i = 0; i < filter.length; i++) {
  switch (filter[i].getFilterCode()) {
    ...
    // 230311 guanshu #562
    case "Suggested":
      filter[i].setCount(countSuggested);
      break;
    // 230313 guanshu #824
    case "Distributed":
      filter[i].setCount(countDistributed);
      break;
    case "TPB Promo":
      filter[i].setCount(countTpbPromoted);
      break;
  }
}
