 ...
 * @function replaceGetJsonQueryForLoItems
 * @this BoOrder
 * @kind businessobject
 * @namespace CUSTOM
...
// 230311 guanshu #562
var considerSuggested = 1;
// 230313 guanshu #824
var considerDistributed = 1;
var considerTpbPromo = 1;
...
orderItemsParams.push( { "field" : "considerSuggested", "value": considerSuggested});
orderItemsParams.push( { "field" : "considerDistributed", "value": considerDistributed});
orderItemsParams.push( { "field" : "considerTpbPromo", "value": considerTpbPromo});
...
if(!orderItemsQuery.useMergeEngine) {
  ...
  // 230311 guanshu #562
  var jsonQuerySuggested = {};
  jsonQuerySuggested.params = [];
  jsonQuerySuggested.params.push({"field" : "customerPKey", "value" : customerPKey});
  jsonQuerySuggested.params.push({"field" : "clbMainPKey", "value" : clbMainPKey});
  // 230313 guanshu #824
  var jsonQueryDistributed = {};
  jsonQueryDistributed.params = [];
  jsonQueryDistributed.params.push({"field" : "clbMainPKey", "value" : clbMainPKey});

  var jsonQueryTpbPromotion = {};
  jsonQueryTpbPromotion.params = [];
  jsonQueryTpbPromotion.params.push({"field" : "customerPKey", "value" : customerPKey});
  ...
  // 230311 guanshu #562
  var suggestedSqlQuery = Facade.myProxy.getLoadStatement("LoMySuggested", jsonQuerySuggested);
  // 230313 guanshu #824
  var distributedSqlQuery = Facade.myProxy.getLoadStatement("LoMyDistributed", jsonQueryDistributed);
  
  var tpbPromotionSqlQuery = Facade.myProxy.getLoadStatement("LoMyTpbPromo", jsonQueryTpbPromotion);
  ...
  // 230311 guanshu #562
  if (considerSuggested == 1) {
    query += "LEFT OUTER JOIN (" + suggestedSqlQuery.sql + ") AS suggested ON suggested.prdMainPKey = product.pKey ";
    queryParams = queryParams.concat(suggestedSqlQuery.params);
  }
  // 230313 guanshu #824
  if (considerDistributed == 1) {
    query += "LEFT OUTER JOIN (" + distributedSqlQuery.sql + ") AS distributed ON distributed.prdMainPKey = product.pKey ";
    queryParams = queryParams.concat(distributedSqlQuery.params);
  }
  if (considerTpbPromo == 1) {
    query += "LEFT OUTER JOIN (" + tpbPromotionSqlQuery.sql + ") AS tpbPromotion ON tpbPromotion.prdMainPKey = product.pKey ";
    queryParams = queryParams.concat(tpbPromotionSqlQuery.params);
  }
