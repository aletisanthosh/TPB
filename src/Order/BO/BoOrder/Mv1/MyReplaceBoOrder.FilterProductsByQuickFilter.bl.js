 ...
 * @function replaceFilterProductsByQuickFilter
 * @this BoOrder
 * @kind businessobject
 * @namespace CUSTOM
 ...
 * @returns items
 */
function replaceFilterProductsByQuickFilter(filterBy, items, filterValue, filterCode){
    var me = this;
    ///////////////////////////////////////////////////////////////////////////////////////////////
    //                                                                                           //
...
var loProductQuickFilter;
var currentItemFilter;
if(Utils.isDefined(filterCode)){
  currentItemFilter = filterCode;
}else{
  currentItemFilter = me.getLoItemFilter().getCurrent().getFilterCode();  
}
...
else if (filterBy === "groupText") {
  if (me.getBoOrderMeta().getConsiderSelectablePromotion() == '1' && Utils.isDefined(me.getLoSelectablePromotion().getCurrent())) {
    currentPromotion = me.getLoSelectablePromotion().getCurrent(); 
  }
  if (!(Utils.isDefined(filterValue) && filterValue[0].filter !== filterBy)) {
    me.getLoItems().resetFilter("groupText");
  }
  ApplicationContext.set('groupTextFilter', items[0].text);
...
if (items[0].specialOption !== "CLEAR") {
  ...
  ApplicationContext.set('groupTextFilter', null);
  filterValue = null;
  if (filterBy === "groupText") {
    me.getLoItems().resetFilter("groupText");
  }
