 ...
 * @function replaceAddItemFromObject
 * @this LoOrderItems
 * @kind listobject
 * @async
 * @namespace CUSTOM
 ...
 * @param {Object} waveForAdd
 * @returns promise
 */
function replaceAddItemFromObject(productsForAdd, productPKey, sdoMainPKey, customerPKey, promotionPkey, considerSelectablePromotion, commitDate, itemTemplate, clbMainPKey, criterionAttribute, barcodeScanBehavior, scanIncrementQuantity, mode, uoM, piecesPerSmallestUnit, orderMeta, loSuggestedQuantity, waveForAdd){
    var me = this;
    ///////////////////////////////////////////////////////////////////////////////////////////////
    //                                                                                           //
...
promise = Facade.getObjectAsync(productInformationCP, productInfoJsonQuery)
  ...
  function (lookupData) {
    ...
    if (!Utils.isDefined(uoM) || Utils.isEmptyString(uoM)) {
      ...
      if (itemTemplate.getQuantityLogisticUnit() == "OrderUnit" || itemTemplate.getQuantityLogisticUnit() == "Order") {
        uoM = productInformationLookup.getOrderQuantityLogisticUnit();
      }
      else if (itemTemplate.getQuantityLogisticUnit() == "ConsumerUnit" || itemTemplate.getQuantityLogisticUnit() == "Consumer") {
        uoM = productInformationLookup.getConsumerQuantityLogisticUnit();
      }
      else if (itemTemplate.getQuantityLogisticUnit() == "PriceUnit" || itemTemplate.getQuantityLogisticUnit() == "Price") {
    ...
    // console.log('considerSelectablePromotion', considerSelectablePromotion); // TODO: remove
    // console.log('waveForAdd', JSON.stringify(waveForAdd)); // TODO: remove
      ...
      existingItems = me.getItemsByParam(
        {
          ...
          "promotionWavePKey" : Utils.isDefined(waveForAdd) && waveForAdd.waveId // TODO: quesetion: do we need to init it with '' empty string by default?
        });
    }
    else {
      existingItems = me.getItemsByParam(
    ...
    if (existingItems.length === 0) {
      ...
      if (Utils.isDefined(product)) {
        ...
        if (Utils.isDefined(waveForAdd)) {
          item.refPKey = productPKey + itemTemplate.getPKey() + ' ' + ' ' + waveForAdd.waveId;
          item.text1 = item.text1 + ' [' + waveForAdd.waveName + ']';
          item.promotionWavePKey = waveForAdd.waveId;
          item.promotionWaveName = waveForAdd.waveName;
        } else {
  ...
  }else if((mode !== 'clone' && mode !== 'massPrebook') && barcodeScanBehavior === "SelectIncrease"){
    var incrementQuantity = 1;
    return me.incrementQuantityByScan(result.unitOfMeasureItem, incrementQuantity);
  }
}).then(function(){
  return result;  
});
